...
def clean_event_text_for_tts(text: str) -> str:
    """Remove speaker labels and event markers from speech chunks before TTS."""
    if not isinstance(text, str):
        return ""

    text = clean_text(text)

    # Qwen sometimes puts these labels inside event_script speech text.
    # They are useful in spoken_transcript, but must NOT be spoken by BreezyVoice.
    remove_patterns = [
        "訪談者：",
        "患者：",
        "訪談者:",
    ...
    # Safety cleanup if Qwen leaves event markers inside a speech chunk.
    text = re.sub(r"\[(停頓|長停頓|嘆氣|咳嗽|吸氣)\]", "", text)
...
"""
    ...
    if cdr_level == "0.5":
        return """
CDR 0.5 / very mild:
- 整體要接近正常長者，只能呈現非常輕微、偶發的不確定。
- 大部分句子要清楚、連貫、能直接回答問題。
- 最多只允許 1 個輕微不確定點，例如日期稍微不確定，或某個小細節想一下。
- 不要同時出現日期錯、地點錯、早餐忘記、藥忘記、家人事件混亂；這樣太像 CDR 1。
- 可以有一點自我修正，但答案最後仍然大致正確。
- 不要大量使用「忘記」、「記不得」、「我想一下」；整段最多出現 1 到 2 次這類詞。
- picture_description: 大多數主要細節都能描述，只漏掉少數小細節，不能像 CDR 1 那樣明顯找詞困難。
- daily_life_description: 生活流程大致清楚，只在一個小地方不確定。
- conversation: 回答大多正確，偶爾需要一點時間想。
- pause_event_count 建議 0 到 2，最多 2；不要使用長停頓，除非真的只有 1 次且很短。
- hesitation_count 建議 1 到 4。
...
- topic_drift_score 0.05 到 0.16。
- coherence_score 0.82 到 0.94。
...
- 可以把時間、地點或事件弄混。
- 句子較破碎，但還要能看出大概意思。
- picture_description: 只描述約 4 到 6 個主要細節；必須漏掉多個物件/動作，至少混淆或不確定 1 個動作。不要完整列出阿公、相簿、眼鏡、茶、小孩、積木、媽媽、電話、貓、茶杯、時鐘、照片全部細節。
...
- For CDR 0.5, mention most major details with only one mild hesitation or uncertainty.
- For CDR 1, mention several details but miss some smaller details.
- For CDR 2, mention only 4 to 6 major details; omit several objects/actions and confuse or be uncertain about at least one action. Do NOT describe the full picture.
...
- event_script speech text must NOT contain speaker labels like 「訪談者：」 or 「患者：」. Use the speaker field only.
