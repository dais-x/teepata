...
VALIDATOR_VERSION = "V8_NO_ACOUSTIC_RECOMPUTED_SCORES_INTERLEAVED_CONVERSATION"
...
- For cognitive_interview: include immediate repetition, orientation answer, then delayed recall answer naturally. The patient-only spoken_transcript may include all patient turns joined together.
...
def rebuild_cognitive_full_conversation(obj: Dict[str, Any]) -> Dict[str, Any]:
    """Interleave interviewer questions and patient answers for cognitive_interview demo audio."""
    ...
    # Split patient speech events into three response regions. This is approximate but prevents
    # the bad pattern where all interviewer questions are spoken before any patient response.
    speech_texts = [str(e.get("text", "")).strip() for e in obj.get("event_script", []) if isinstance(e, dict) and e.get("type") == "speech" and str(e.get("text", "")).strip()]
    if len(speech_texts) >= 3:
        patient_chunks = [speech_texts[0], "".join(speech_texts[1:-1]), speech_texts[-1]]
    elif len(speech_texts) == 2:
        patient_chunks = [speech_texts[0], "", speech_texts[1]]
    elif len(speech_texts) == 1:
        # Use heuristic split by delayed-recall anchor if possible.
        ...
        patient_chunks = [head[:80].strip(), head[80:].strip(), tail.strip()]
    else:
        patient_chunks = ["", "", ""]

    events = []
    context = []
    for q, ans in zip(prompts, patient_chunks):
    ...
    # Remove trailing pause from demo events.
    if events and events[-1].get("type") == "pause":
        events.pop()
...
def repair_speech_cognitive_task_from_text(obj: Dict[str, Any], cdr: str) -> Dict[str, Any]:
    ...
    if task_group == "picture_description":
        ...
        imm_text = immediate_repetition_region(spoken)
        delayed_text = delayed_recall_region(spoken)
        immediate_items = count_recalled_items(imm_text, items)
        delayed_items = count_recalled_items(delayed_text, items)
        imm_score = len(immediate_items)
        delayed_score = len(delayed_items)
        # Orientation truth cannot be inferred perfectly from text, so use Qwen's count but clamp.
        orientation_score = task.get("orientation_correct_count", None)
        if not isinstance(orientation_score, int):
            orientation_score = task.get("subtasks", {}).get("orientation", {}).get("score", 0) if isinstance(task.get("subtasks"), dict) else 0
        orientation_score = max(0, min(3, int(orientation_score or 0)))
...
def validate_sample(obj: Dict[str, Any], expected: Dict[str, Any]) -> Tuple[bool, List[str]]:
    ...
    if not isinstance(task, dict):
        ...
        if expected.get("task_group") == "picture_description":
            ...
            delayed_items = count_recalled_items(delayed_recall_region(spoken), COGNITIVE_INTERVIEW_TASK["expected_items"])
            delayed_score = int(subtasks.get("delayed_recall", {}).get("score", 0) or 0)
            if delayed_score != len(delayed_items):
                errors.append(f"delayed_recall score={delayed_score} does not match transcript-derived recalled items {len(delayed_items)}")
            if task.get("patient_recalled_items") != delayed_items:
                errors.append("patient_recalled_items must match transcript-derived delayed recall items")
        elif expected.get("task_group") == "real_life_scenarios":
